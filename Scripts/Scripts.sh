...
function location ()
{
    ...
    city=$(redis-client 6 GET city) # do a SET operation
	exec 6>&- # close the connection
	curl -s "https://query.yahooapis.com/v1/public/yql?q=select%20*%20from%20weather.forecast%20where%20woeid%20in%20%28select%20woeid%20from%20geo.places%281%29%20where%20text%3D%22$city%2Cir%22%29&format=xml&env=store%3A%2F%2Fdatatables.org%2Falltableswithkeys" -o ~/.cache/weather.xml
}
